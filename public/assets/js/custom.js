...
(function ($) {
    ...
    // ponytail: template shipped without thmOwlInit defined -> ReferenceError killed the whole load handler.
    // Define it here so owl carousels (e.g. Area We Serve) init and later load-handler code still runs.
    if (typeof window.thmOwlInit !== "function") {
      window.thmOwlInit = function () {
        if (window.jQuery && $.fn.owlCarousel && $(".area-slider").length) {
          $(".area-slider").owlCarousel({
            loop: true,
            margin: 20,
            autoplay: true,
