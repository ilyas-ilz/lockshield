...
const WidgetModel = model<IWidget>(
  "CrudWidget",
  new Schema<IWidget>({ name: String, qty: Number, status: String }, { timestamps: true })
);

const createSchema = z.object({ name: z.string().min(1), qty: z.number().int().default(0), status: z.string().optional() });
...
describe("CRUD factory (via Widget)", () => {
  ...
  it("filters by ?status= when status is present in query", async () => {
    await WidgetModel.create({ name: "DocPublished", qty: 1, status: "published" });
    await WidgetModel.create({ name: "DocDraft", qty: 1, status: "draft" });
    const res = await listGET(req("/api/widgets?status=published"));
    const body = await res.json();
    expect(body.data.items).toHaveLength(1);
    expect(body.data.items[0].name).toBe("DocPublished");
  });
